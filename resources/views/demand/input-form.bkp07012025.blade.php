...
<div class="card">
    <div class="card-body">
        ...
        <div class="col col-lg-2 pt-1 mb-2 {{isset($demand) ? 'd-none':''}}">
            <button type="button" class="btn btn-primary px-4 mt-4" id="submitButton">Continue<i class="bx bx-right-arrow-alt ms-2"></i></button>
        </div>
        ...
        <div class="d-none" id="detail-card">
            ...
            <button type="button" class="btn btn-primary mb-2" id="btn-demand" data-action="show">Create Demand</button>
        ...
        <div class="{{isset($demand) ? '':'d-none'}}" id="input-form-container">
            ...
            <form id="demand-input-form" method="post" action="">
                ...
                <div id="subhead-container">
                    @if(isset($demand))
                    @foreach ($demand->demandDetails as $key=>$detail)
                    <div class="row mt-2 subhead-input">
                        ...
                        <div class="col-lg-4">
                            <div class="form-group" style="margin-right: 0px; margin-bottom: 5px;">
                                ...
                                <select name="subhead[]" id="subhead-1" class="form-select" value="{{$detail->subhead_id}}">
                                    <option value="">Select</option>
                                    @foreach ($demandSubheads as $subhead)

                                    @php
                                    if(!is_null($subhead->additional_data))
                                    {
                                    $additional_data = json_decode($subhead->additional_data);
                                    if(isset($additional_data->duration)){
                                    $setDurationRequired = true;
                                    }
                                    if(isset($additional_data->remarks)){
                                    $setRemarksRequired = true;
                                    }
                                    }
                                    @endphp
                                    <option value="{{$subhead->id}}" {{ $subhead->id == $detail->subhead_id ? 'selected':''}} {{ isset($setDurationRequired) ? 'attrDuration="true"':'' }} {{ isset($setRemarksRequired) ? 'attrRemarks="true"':'' }}>
                                        {{$subhead->item_name}}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="amount[]" id="subhead_amount-1" min="0" step="0.01" required value="{{$detail->total}}" />
                            </div>
                        </div>
                        <div class="col-lg-4">
                            ...
                            <div class="mix-field">
                                <div>
                                    ...
                                    <input type="date" class="form-control" name="duration_from[]" id="subhead_duration-1" value="{{$detail->duration_from}}">
                                </div>
                                <span>-</span>
                                <div>
                                    <label for="toduration">To</label>
                                    <input type="date" class="form-control" name="duration_to[]" id="subhead_duration-1" value="{{$detail->duration_from}}">
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group" style="margin-bottom: 5px;">
                                ...
                                <div class="mix-field">
                                    <textarea name="remark[]" id="subhead_remark-1" class="form-control">{{$detail->remarks}}</textarea>
                    ...
                    @endforeach
                    @else
                    <div class="row mt-2 align-items-center subhead-input">
                        <!-- <div class="col-lg-4 ">
                        ...
                        <div class="col-lg-4">
                            <div class="form-group" style="margin-right: 0px; margin-bottom: 5px;">
                                <label for="subhead_input">Subhead</label>
                                <select name="subhead[]" id="subhead-1" class="form-select">
                                    <option value="">Select</option>
                                    @foreach ($demandSubheads as $subhead)
                                    @php
                                    if(!is_null($subhead->additional_data))
                                    ...
                                    @endphp
                                    <option value="{{$subhead->id}}" {{ isset($setDurationRequired) ? 'attrDuration="true"':'' }} {{ isset($setRemarksRequired) ? 'attrRemarks="true"':'' }}>{{$subhead->item_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="form-group" style="margin-bottom: 5px;">
                                <label for="amount">Amount</label>
                                <input type="number" class="form-control" name="amount[]" id="subhead_amount-1" min="0" step="0.01" required>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="form-group" style="margin-bottom: 5px;">
                                ...
                                <div class="mix-field">
                                    <div>
                                        <label for="fromduration">From</label>
                                        <input type="date" class="form-control" name="duration_from[]" id="subhead_duration-1">
                                    </div>
                                    <span>-</span>
                                    <div>
                                        <label for="toduration">To</label>
                                        <input type="date" class="form-control" name="duration_to[]" id="subhead_duration-1">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group" style="margin-bottom: 5px;">
                                <label for="amount">Remark</label>
                                <div class="mix-field">
                                    <textarea name="remark[]" id="subhead_remark-1" class="form-control" rows="4"></textarea>
                                    <button type="button" class="btn btn-danger btn-remove-subhead ml-2"><i class='bx bx-trash'></i></button>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content text-center">
            ...
            <div class="modal-footer border-0 justify-content-center">
                ...
                <button type="button" name="status" value="submit" class="btn btn-warning btn-width" id="confirmation-yes">Yes</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function appendPropertyDetail(data, isMultiple = false, rowNum = null) {
        ...
        if (data.trasferDetails && data.trasferDetails.length > 0) {
            ...
            data.trasferDetails.forEach((row, i) => {
                transferHTML += `<li class="transfer-list-item row row-lg-4">
                    ...
                    <div class="transfer-list-cell col">${
                      row.transferDate
                    }</div>
                    <div class="transfer-list-cell col">${
                      row.process_of_transfer
                    }</div>
        ...
        if (data.rgr_id) {
            $("#detail-card").prepend(
                `<div id="view-draft"><a class="btn btn-success"  onclick="viewDraft(${data.rgr_id})">View Draft</a></div>`
            );
        }
    }
    ...
    $("#btn-demand").click(function() {
        ...
        if (selectedOldPropertyId && selectedOldPropertyId != "") {
            $.ajax({
                type: "get",
                url: "{{url('/demand/getExistingPropertyDemand')}}" +
                    "/" +
                    selectedOldPropertyId,
                success: function(response) {
                    if (response.status) {
                        if (response.data && response.data.demand) {
              ...
              <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <th>${oldDemand.net_total}</th>
                    <th>${oldDemand.balance_amount}</th>
                </tr>
            </table>
            `;
    ...
    $(document).on("click", ".btn-remove-subhead", function() {
        console.log("function called", $(".subhead-input").length);
    ...
    $("#btn-submit").click(function() {
        ...
        $("#demand-input-form").find("[required]").each(function() {
            if ($(this).val() === "" || $(this).val() === null) {
                isValid = false;
                errorMessage += $(this).closest('.form-group').find('label').text() + " is required.\n";
            }
        });
    ...
    $(document).on('change', 'select[name="subhead[]"]', function() {
        ...
        if (selectedOption.attr('attrRemarks')) {
            ...
            subheadContainer.find('textarea[name="remark[]"]').parent().siblings('label').html('Remark <span class="text-danger">*</span>');
        }
    })
</script>
@endsection