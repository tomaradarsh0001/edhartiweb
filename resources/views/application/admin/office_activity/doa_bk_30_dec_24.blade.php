                ...
                {{-- @if ($currentDateFormatted == $applicationAppointmentLink['schedule_date'])
                    <div class="col-lg-8 mt-4">
                        <a href="{{ route('startProofReading', ['id' => $details->id]) }}?type={{ request()->query('type') }}"
                            target="_blank">
                            <button type="button" data-bs-toggle="modal" data-bs-target="#startProofReadingModal" id="PropertyIDSearchBtn" class="btn btn-primary ml-2">Start Proof Reading</button>
                        </a>
                    </div>
                @endif --}}
                ...
                @if ($showActionButtons)
                    ...
                    @if ($roles === 'deputy-lndo')
                        @if ($showAppointmentLinkButton)
                            <button type="button" id="sendProofReadingLink" class="btn btn-primary" onclick="handleApplicationAction('PROOFREADINGLINK','{{ $details->application_no }}',this)">Send Proof Reading Link</button>
                        @endif
