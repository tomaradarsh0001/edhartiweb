    ...
    <div class="row">
        <div class="d-flex justify-content-end gap-4 col-lg-12" id="action-btn-container">
            @if ($showActionButtons)
                @if ($roles === 'section-officer')
                    @if (!$showRecommandForAppoval && !$showRecommandAndObjectButtonSection)
                    ...
                    @if($isAppointmentAttended)
                        @if(getStatusDetailsById( $details->status ?? '' )->item_code != 'HOLD')
                            @if($showRecommandForAppoval)
                                <button type="button" class="btn btn-primary" onclick="handleApplicationAction('RECOMMENDED_FOR_APPROVAL','{{ $details->application_no }}',this)">Recommend For Approval</button>
                            @endif
